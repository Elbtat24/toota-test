...
// محاضرات المواد (Firestore + Storage)
const FIRESTORE_LECTURES_COLLECTION = 'lectures_v1';
const FIRESTORE_LECTURES_SUBCOLLECTION = 'files_v1';
const STORAGE_LECTURES_ROOT = 'lectures_v1';
...
function closeModal(modalId) {
    // تنظيف اشتراكات المحاضرات عند الإغلاق
    if (modalId === 'lecturesModal') {
        if (__lecturesModalUnsub) { try { __lecturesModalUnsub(); } catch (e) {} __lecturesModalUnsub = null; }
        __lecturesCurrentSubject = null;
        const list = document.getElementById('lecturesModalList');
        const status = document.getElementById('lecturesModalStatus');
...
if (sectionId === 'settings') {
    ...
    if (isAdminUnlocked()) {
        ...
        const l = document.getElementById('adminLecturesList');
        if (l) l.innerHTML = '';
        const s = document.getElementById('lecturesUploadStatus');
        if (s) s.textContent = '';
    }
}
}
...
function lockAdminAccess() {
    ...
    // إلغاء الاشتراكات (لو موجودة)
    if (__lecturesAdminUnsub) { try { __lecturesAdminUnsub(); } catch (e) {} __lecturesAdminUnsub = null; }
    if (__lecturesModalUnsub) { try { __lecturesModalUnsub(); } catch (e) {} __lecturesModalUnsub = null; }
    __lecturesCurrentSubject = null;

    // إغلاق نافذة المحاضرات لو مفتوحة
...
function openLectures(subject) {
    ...
    const modal = document.getElementById('lecturesModal');
    if (!modal) {
        showAlert('نافذة المحاضرات غير موجودة في الصفحة.', 'error');
        return;
    }

    __lecturesCurrentSubject = subjectId;

    const title = document.getElementById('lecturesModalTitle');
    if (title) title.innerHTML = `<i class="fas fa-book"></i> محاضرات ${escapeHtml(getSubjectName(subjectId))}`;

    const status = document.getElementById('lecturesModalStatus');
    const list = document.getElementById('lecturesModalList');
    if (list) list.innerHTML = '';
    if (status) status.innerHTML = `<span class="loading"></span> جاري تحميل المحاضرات...`;

    modal.style.display = 'flex';
    loadLecturesModalList(subjectId);
}

// ==========================================
// Lectures System (Firebase Storage + Firestore)
// ==========================================

let __lecturesModalUnsub = null;
let __lecturesAdminUnsub = null;
let __lecturesCurrentSubject = null;

function isStorageReady() {
    return !!(window.firebaseStorage && window.storageApi);
}
...
function getLecturesSubjectsFromUI() {
    const out = [];
    document.querySelectorAll('.subject-card[data-subject]').forEach(card => {
        const code = String(card.getAttribute('data-subject') || '').trim();
        if (!code) return;
        const h = card.querySelector('h3');
        const name = (h ? h.textContent : '').trim() || getSubjectName(code);
        if (!out.some(x => x.code === code)) out.push({ code, name });
    });
    return out;
}

function lecturesFilesCollectionRef(subjectId) {
    if (!isFirestoreReady()) return null;
    const api = window.firestoreApi;
    const db = window.firestoreDb;
    const sid = String(subjectId || '').trim();
    if (!sid) return null;
    return api.collection(db, FIRESTORE_LECTURES_COLLECTION, sid, FIRESTORE_LECTURES_SUBCOLLECTION);
}

function getFileIconClass(fileName) {
    const n = String(fileName || '').toLowerCase();
    if (n.endsWith('.pdf')) return 'fas fa-file-pdf';
    if (n.endsWith('.ppt') || n.endsWith('.pptx')) return 'fas fa-file-powerpoint';
    if (n.endsWith('.doc') || n.endsWith('.docx')) return 'fas fa-file-word';
    if (n.endsWith('.xls') || n.endsWith('.xlsx') || n.endsWith('.csv')) return 'fas fa-file-excel';
    if (n.endsWith('.zip') || n.endsWith('.rar') || n.endsWith('.7z')) return 'fas fa-file-archive';
    if (n.endsWith('.mp4') || n.endsWith('.mov') || n.endsWith('.mkv')) return 'fas fa-file-video';
    if (n.endsWith('.mp3') || n.endsWith('.wav')) return 'fas fa-file-audio';
    if (n.endsWith('.png') || n.endsWith('.jpg') || n.endsWith('.jpeg') || n.endsWith('.webp')) return 'fas fa-file-image';
    return 'fas fa-file';
}

function renderLecturesList(container, docs, { showDelete = false, subjectId = '' } = {}) {
    if (!container) return;

    if (!Array.isArray(docs) || docs.length === 0) {
        container.innerHTML = `
            <div class="no-results" style="margin-top: 10px;">
                <i class="fas fa-inbox"></i>
    ...
    container.innerHTML = docs.map(d => {
        const fileName = escapeHtml(String(d.fileName || 'ملف'));
        const url = escapeHtml(String(d.downloadURL || ''));
        const nameHtml = url ? `<a class="lecture-name lecture-name-link" href="${url}" target="_blank" rel="noopener" download title="${fileName}">${fileName}</a>` : `${nameHtml}`;
        const size = escapeHtml(formatBytes(d.sizeBytes || 0));
        const icon = getFileIconClass(d.fileName);
        const docId = escapeHtml(String(d.id || ''));
        const storagePath = escapeHtml(String(d.storagePath || ''));

        const metaText = [size].filter(Boolean).join(' • ');
        const downloadBtn = url
            ? `<a class="lecture-btn" href="${url}" target="_blank" rel="noopener" download>
                   <i class="fas fa-download"></i> تنزيل
               </a>`
            : `<button class="lecture-btn" disabled>
                   <i class="fas fa-ban"></i> لا يوجد رابط
               </button>`;

        const deleteBtn = showDelete
            ? `<button class="lecture-btn delete" onclick="deleteLectureFileAdmin('${escapeHtml(subjectId)}','${docId}','${storagePath}')">
        ...
        return `
            <div class="lecture-item">
                <div class="lecture-left">
                    <div class="lecture-icon"><i class="${icon}"></i></div>
                    <div style="min-width:0;">
                        ${nameHtml}
                        <div class="lecture-meta">${metaText}</div>
                    </div>
                </div>
...
async function loadLecturesModalList(subjectId, { forceRefresh = false } = {}) {
    const status = document.getElementById('lecturesModalStatus');
    const list = document.getElementById('lecturesModalList');

    if (!isFirestoreReady()) {
        if (status) status.textContent = 'Firebase Firestore غير جاهز. تأكد إنك فعّلت Firestore في Firebase Console.';
        if (list) list.innerHTML = '';
        return;
    }

    const col = lecturesFilesCollectionRef(subjectId);
    if (!col) {
        if (status) status.textContent = 'لا يمكن تحميل محاضرات هذه المادة.';
        return;
    }
    ...
    const render = (docSnaps) => {
        const items = [];
        (docSnaps || []).forEach(s => items.push({ id: s.id, ...(s.data ? s.data() : {}) }));
        renderLecturesList(list, items, { showDelete: false, subjectId });
    ...
    try {
        ...
        if (api.onSnapshot && !forceRefresh) {
            __lecturesModalUnsub = api.onSnapshot(q, (snap) => {
                render(snap.docs || []);
            }, (err) => {
                console.error(err);
                if (status) status.textContent = 'حصل خطأ أثناء تحميل المحاضرات.';
            });
        } else {
            const snap = await api.getDocs(q);
            render(snap.docs || []);
...
// =====================
// Admin UI (رفع المحاضرات)
// =====================

function initLecturesAdminUI() {
    const select = document.getElementById('lecturesSubjectSelect');
    if (!select) return;

    // Fill subjects
    const subjects = getLecturesSubjectsFromUI();
    if (subjects.length) {
        select.innerHTML = subjects.map(s => `<option value="${escapeHtml(s.code)}">${escapeHtml(s.name)}</option>`).join('');
    }

    const saved = String(localStorage.getItem('spacePlatform_adminLecturesSubject') || '').trim();
    if (saved && subjects.some(s => s.code === saved)) {
        select.value = saved;
    }

    if (!select.dataset.bound) {
        select.addEventListener('change', () => {
            localStorage.setItem('spacePlatform_adminLecturesSubject', String(select.value || ''));
            refreshAdminLecturesList(true);
        });
...
async function refreshAdminLecturesList(forceRefresh = false) {
    const status = document.getElementById('lecturesUploadStatus');
    ...
    if (!isCurrentUserAdminRole()) {
        status.textContent = 'للأدمن فقط.';
        list.innerHTML = '';
        return;
    }
    if (!isAdminUnlocked()) {
        status.textContent = 'لازم تفتح الأدمن الأول.';
        list.innerHTML = '';
        return;
    }

    const subjectId = String(select.value || '').trim();
    if (!subjectId) {
        status.textContent = 'اختر المادة أولاً.';
        list.innerHTML = '';
        return;
    }

    if (!isFirestoreReady()) {
        status.textContent = 'Firebase Firestore غير جاهز.';
        list.innerHTML = '';
        return;
    }
    ...
    status.innerHTML = `<span class="loading"></span> جاري تحميل ملفات المادة...`;
    ...
    const render = (docSnaps) => {
        const items = [];
        (docSnaps || []).forEach(s => items.push({ id: s.id, ...(s.data ? s.data() : {}) }));
        renderLecturesList(list, items, { showDelete: true, subjectId });
        status.textContent = items.length ? `عدد الملفات: ${items.length}` : 'لا توجد ملفات.';
    };
    ...
    try {
        ...
        if (api.onSnapshot && !forceRefresh) {
            __lecturesAdminUnsub = api.onSnapshot(q, (snap) => {
                render(snap.docs || []);
            }, (err) => {
                console.error(err);
        ...
        console.error(e);
        status.textContent = 'حصل خطأ أثناء تحميل الملفات.';
    }
}

async function uploadLecturesFiles() {
    if (!isCurrentUserAdminRole()) {
        showAlert('للأدمن فقط.', 'error');
        return;
    }
    ...
    const select = document.getElementById('lecturesSubjectSelect');
    const input = document.getElementById('lecturesFilesInput');
    const status = document.getElementById('lecturesUploadStatus');
    const list = document.getElementById('adminLecturesList');
    ...
    if (!subjectId) {
        showAlert('اختار المادة الأول.', 'error');
        return;
    }
    if (!files.length) {
        showAlert('اختار ملف أو أكتر للرفع.', 'error');
        return;
    }

    if (!isFirestoreReady()) {
        showAlert('Firebase Firestore غير جاهز.', 'error');
        return;
    }
    if (!isStorageReady()) {
        showAlert('Firebase Storage غير جاهز. تأكد إنك ضفت firebase-storage.js في index.html.', 'error');
        return;
    }

    const api = window.firestoreApi;
    const storage = window.firebaseStorage;
    const sApi = window.storageApi;

    const col = lecturesFilesCollectionRef(subjectId);
    if (!col) return;

    if (status) status.innerHTML = `<span class="loading"></span> جاري رفع الملفات...`;
    if (list) list.innerHTML = '';

    const progressWrap = document.createElement('div');
    progressWrap.className = 'lectures-list';
    if (list) list.appendChild(progressWrap);
    ...
    const makeRow = (name) => {
        ...
        row.innerHTML = `
            <div class="lecture-left">
                <div class="lecture-icon"><i class="${getFileIconClass(name)}"></i></div>
                <div style="min-width:0;">
                    <div class="lecture-name">${escapeHtml(name)}</div>
                    <div class="lecture-meta">جاري الرفع…</div>
                </div>
            </div>
            <div class="lecture-actions">
                <div class="lecture-progress"><span></span></div>
            </div>
        `;
    ...
    for (const file of files) {
        ...
        const row = makeRow(cleanName);
        const metaEl = row.querySelector('.lecture-meta');
        const bar = row.querySelector('.lecture-progress > span');

        try {
            const unique = `${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
            const storagePath = `${STORAGE_LECTURES_ROOT}/${subjectId}/${unique}_${cleanName}`;
            const ref = sApi.ref(storage, storagePath);

            const task = sApi.uploadBytesResumable(ref, file, {
            ...
            const url = await new Promise((resolve, reject) => {
                task.on('state_changed',
                    (snap) => {
                        ...
                        if (metaEl) metaEl.textContent = `جاري الرفع… ${pct}%`;
                    },
                    (err) => reject(err),
                    async () => {
                        try {
                            const downloadURL = await sApi.getDownloadURL(task.snapshot.ref);
                            resolve(downloadURL);
                        } catch (e) {
                            reject(e);
                        }
            ...
            // Save metadata in Firestore (visible to all users)
            await api.addDoc(col, {
                fileName: cleanName,
                originalFileName: String(file.name || cleanName),
                subjectId,
                downloadURL: String(url || ''),
                storagePath,
                sizeBytes: Number(file.size || 0),
                contentType: String(file.type || ''),
                uploadedByUid: String(appData?.firebaseUser?.uid || ''),
                uploadedByName: String(appData?.firebaseUser?.displayName || appData?.currentUser?.name || ''),
                createdAtMs: Date.now(),
                createdAt: api.serverTimestamp()
            });

            if (metaEl) metaEl.textContent = `تم الرفع ✅ (${formatBytes(file.size)})`;
            results.ok++;
        } catch (e) {
            console.error('Upload lecture failed:', e);
            if (metaEl) metaEl.textContent = 'فشل الرفع ❌';
            results.fail++;
        }
    }

    if (status) status.textContent = `تم رفع ${results.ok} ملف${results.ok === 1 ? '' : ''}، وفشل ${results.fail}.`;
    if (input) input.value = '';

    // refresh list (real list)
    refreshAdminLecturesList(true);

    if (results.ok) showAlert('تم رفع المحاضرات ✅', 'success');
    if (results.fail) showAlert('بعض الملفات لم تُرفع. راجع الحجم/الصلاحيات.', 'info');
}

async function deleteLectureFileAdmin(subjectId, docId, storagePath) {
    if (!isCurrentUserAdminRole()) {
        showAlert('للأدمن فقط.', 'error');
        return;
    }
    ...
    if (!subjectId || !docId) return;

    if (!confirm('متأكد إنك عايز تحذف الملف؟')) return;

    if (!isFirestoreReady()) {
        showAlert('Firestore غير جاهز.', 'error');
        return;
    }
    ...
    try {
        // delete from storage first (optional)
        if (isStorageReady() && storagePath) {
            const sApi = window.storageApi;
            const storage = window.firebaseStorage;
            const ref = sApi.ref(storage, storagePath);
            await sApi.deleteObject(ref);
        }

        const docRef = api.doc(db, FIRESTORE_LECTURES_COLLECTION, subjectId, FIRESTORE_LECTURES_SUBCOLLECTION, docId);
        ...
        console.error(e);
        showAlert('فشل الحذف. راجع صلاحيات Firestore/Storage Rules.', 'error');
    }
}
// ==========================================
// AI Assistant System
