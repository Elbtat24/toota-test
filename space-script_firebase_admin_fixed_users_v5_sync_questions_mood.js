...
// =====================
// موسيقى المود (قائمة مشتركة لكل المستخدمين)
// =====================
const MOOD_PLAYLIST_KEY = 'spacePlatform_moodPlaylist_v1';
const MOOD_PLAYLIST_FIRESTORE_TYPE = 'moodPlaylist';

// حالة مشغل الصوت
let __moodPlaylist = null;
let __moodPlaylistDraft = null;
let __moodPlaylistUnsub = null;

let __moodAudioEl = null;
let __moodTrackIndex = 0;
let __moodUiBound = false;
let __moodIsPlaying = false;
...
// =====================
// موسيقى المود (تشغيل أغاني من قائمة موحّدة لكل المستخدمين)
// =====================

function normalizeMoodPlaylist(list) {
    if (!Array.isArray(list)) return [];
    const out = [];
    list.forEach((it) => {
        const url = String(it?.url || '').trim();
        if (!url) return;
        const name = String(it?.name || it?.title || 'أغنية').trim() || 'أغنية';
        out.push({ name, url });
    });
    return out;
}

function loadMoodPlaylistFromLocal() {
    try {
        const raw = localStorage.getItem(MOOD_PLAYLIST_KEY);
        if (!raw) return [];
        return normalizeMoodPlaylist(JSON.parse(raw));
    } catch (e) {
        return [];
    }
}

function persistMoodPlaylistToLocal(list) {
    try {
        localStorage.setItem(MOOD_PLAYLIST_KEY, JSON.stringify(normalizeMoodPlaylist(list)));
    } catch (e) {}
}

function getMoodPlaylist() {
    if (!__moodPlaylist) __moodPlaylist = loadMoodPlaylistFromLocal();
    return __moodPlaylist;
}

function setMoodPlaylist(list, { render = true } = {}) {
    __moodPlaylist = normalizeMoodPlaylist(list);
    persistMoodPlaylistToLocal(__moodPlaylist);

    // لو مفيش أغاني، أوقف المشغل
    if (__moodPlaylist.length === 0) {
        stopMoodMode();
    } else {
        // لو المؤشر خرج برة القائمة، رجّعه للصفر
        if (__moodTrackIndex >= __moodPlaylist.length) __moodTrackIndex = 0;
    }

    if (render) {
        updateMoodModeBtnUI();
        renderMoodPlaylistEditor();
    }
}

function ensureMoodAudioEl() {
    if (__moodAudioEl) return __moodAudioEl;
    __moodAudioEl = document.getElementById('moodAudio');

    if (!__moodAudioEl) {
        // fallback لو حد شال الـ audio من html
        __moodAudioEl = document.createElement('audio');
        __moodAudioEl.id = 'moodAudio';
        __moodAudioEl.preload = 'none';
        document.body.appendChild(__moodAudioEl);
    }

    __moodAudioEl.addEventListener('ended', onMoodTrackEnded);
    __moodAudioEl.addEventListener('pause', () => {
        __moodIsPlaying = false;
        updateMoodModeBtnUI();
    });
    __moodAudioEl.addEventListener('play', () => {
        __moodIsPlaying = true;
        updateMoodModeBtnUI();
    });

    return __moodAudioEl;
}

function initMoodModeUIOnce() {
    if (__moodUiBound) return;
    __moodUiBound = true;

    // حمّل آخر قائمة محلياً (حتى قبل الـ sync)
    getMoodPlaylist();
...
function updateMoodModeBtnUI() {
    const btn = document.getElementById('moodModeBtn');
    if (!btn) return;

    const list = getMoodPlaylist();
    const hasSongs = list.length > 0;

    btn.disabled = !hasSongs;
    btn.classList.toggle('playing', !!__moodIsPlaying);

    // Tooltip بسيط
    const current = list[__moodTrackIndex];
    const title = hasSongs
        ? `🎵 ${current ? current.name : ''}\nاضغط للتشغيل/الإيقاف`
        : 'مفيش أغاني متاحة دلوقتي (استنى الأدمن يرفع قائمة).';
    btn.title = title;
}

function stopMoodMode() {
    try {
        const a = ensureMoodAudioEl();
        a.pause();
...
function playMoodTrackAt(index, { fromUser = false } = {}) {
    const list = getMoodPlaylist();
    if (!Array.isArray(list) || list.length === 0) {
        updateMoodModeBtnUI();
        showAlert('مفيش أغاني متاحة دلوقتي. خلي الأدمن يرفع قائمة.', 'info');
        return;
    }

    __moodTrackIndex = Math.max(0, Math.min(list.length - 1, parseInt(index, 10) || 0));
    const track = list[__moodTrackIndex];
    ...
    try {
        ...
        a.play().catch((err) => {
            ...
            if (fromUser) {
                showAlert('متصفحك منع التشغيل التلقائي. اضغط الزرار مرة تانية للتشغيل.', 'info');
            }
        });
    } catch (e) {
        console.warn(e);
...
function playNextMoodTrack() {
    ...
    const next = (__moodTrackIndex + 1) % list.length;
    playMoodTrackAt(next);
}

function onMoodTrackEnded() {
    // شغّل اللي بعدها تلقائياً
    playNextMoodTrack();
}
...
function toggleMoodMode() {
    ...
    // لو مفيش src، ابدأ من أول قائمة
    if (!a.src) {
        playMoodTrackAt(0, { fromUser: true });
    } else {
        a.play().catch((err) => {
            console.warn('Audio play blocked:', err);
            showAlert('اضغط الزرار مرة تانية للتشغيل.', 'info');
        });
    }
    updateMoodModeBtnUI();
}
...
function isStorageReady() {
    return !!(window.firebaseStorage && window.storageApi && typeof window.storageApi.uploadBytes === 'function');
}

async function saveMoodPlaylistToFirestore(playlist) {
    try {
        if (!isFirestoreReady()) return false;
        ...
        const payload = {
            type: MOOD_PLAYLIST_FIRESTORE_TYPE,
            value: normalizeMoodPlaylist(playlist),
            createdAtMs: Date.now(),
            createdAt: api.serverTimestamp()
        };
        ...
        console.warn('Failed to save mood playlist to Firestore:', e);
        return false;
    }
}

async function syncMoodPlaylistFromFirestore() {
    try {
        if (!isFirestoreReady()) return false;
        ...
        // أحدث وثيقة من النوع moodPlaylist
        const q = api.query(
            api.collection(db, FIRESTORE_CONFIG_COLLECTION),
            api.where('type', '==', MOOD_PLAYLIST_FIRESTORE_TYPE),
            api.orderBy('createdAtMs', 'desc'),
            api.limit(1)
        );
        ...
        snap.forEach((docSnap) => {
            const d = docSnap.data();
            if (!latest && d && Array.isArray(d.value)) {
                latest = d.value;
            }
        });

        if (latest) {
            setMoodPlaylist(latest);
            return true;
        }
        return false;
    } catch (e) {
        console.warn('Failed to sync mood playlist from Firestore:', e);
        return false;
    }
}

function initMoodPlaylistSync() {
    if (!isFirestoreReady()) return;
    if (__moodPlaylistUnsub) return; // already
    ...
    try {
        __moodPlaylistUnsub = api.onSnapshot(q, (snap) => {
            let latest = null;
            ...
            if (latest) {
                ...
                // fallback: لو مفيش، خليك على المحلي
                renderMoodPlaylistEditor();
                updateMoodModeBtnUI();
            }
        }, (err) => {
            console.warn('Mood playlist onSnapshot error:', err);
        });
    } catch (e) {
        console.warn(e);
    }

    // fetch once too (مفيد لأول مرة)
    syncMoodPlaylistFromFirestore();
}

function stopMoodPlaylistSync() {
    try {
        if (typeof __moodPlaylistUnsub === 'function') __moodPlaylistUnsub();
    } catch (e) {}
    __moodPlaylistUnsub = null;
}

// ====== Admin UI (رفع الأغاني وترتيبها) ======

function requireAdminForMoodEdit() {
...
function initMoodAdminUI() {
    if (!isCurrentUserAdminRole()) return;
    if (!isAdminUnlocked()) return;

    if (!__moodPlaylistDraft) {
        __moodPlaylistDraft = JSON.parse(JSON.stringify(getMoodPlaylist() || []));
    }
    renderMoodPlaylistEditor();
}

function setMoodUploadStatus(htmlText) {
    const el = document.getElementById('moodUploadStatus');
    if (!el) return;
    el.innerHTML = htmlText || '';
}

function renderMoodPlaylistEditor() {
    const editor = document.getElementById('moodPlaylistEditor');
    if (!editor) return;

    // لو الأدمن فاتح، اعرض Draft، وإلا اعرض القائمة الحالية للعرض فقط
    const unlocked = isCurrentUserAdminRole() && isAdminUnlocked();
    const list = unlocked ? (__moodPlaylistDraft || []) : (getMoodPlaylist() || []);

    if (!Array.isArray(list) || list.length === 0) {
        editor.innerHTML = `<div class="settings-note" style="margin:0;">مفيش أغاني مرفوعة لسه.</div>`;
        updateMoodModeBtnUI();
        return;
    }

    const rows = list.map((t, idx) => {
        const name = escapeHtml(t?.name || `Track ${idx + 1}`);
        const upBtn = unlocked
            ? `<button class="mood-mini-btn" onclick="moveMoodTrack(${idx},-1)" title="فوق">↑</button>`
            : '';
        const downBtn = unlocked
            ? `<button class="mood-mini-btn" onclick="moveMoodTrack(${idx},1)" title="تحت">↓</button>`
            : '';
        const delBtn = unlocked
            ? `<button class="mood-mini-btn danger" onclick="removeMoodTrack(${idx})" title="حذف">حذف</button>`
            : '';

        return `
            <div class="mood-track-row">
                <div class="mood-track-index">${idx + 1}</div>
                <div class="mood-track-name" title="${name}">${name}</div>
...
function moveMoodTrack(index, dir) {
    if (!requireAdminForMoodEdit()) return;

    const list = Array.isArray(__moodPlaylistDraft) ? __moodPlaylistDraft : [];
    ...
    __moodPlaylistDraft = list;
    renderMoodPlaylistEditor();
}

function removeMoodTrack(index) {
    if (!requireAdminForMoodEdit()) return;
...
async function uploadSingleMoodFile(file) {
    const storage = window.firebaseStorage;
    const api = window.storageApi;
    const cleanName = String(file?.name || 'track').replace(/[^\w\u0600-\u06FF\-. ]+/g, '_');
    const path = `mood_songs/${Date.now()}_${Math.random().toString(16).slice(2)}_${cleanName}`;

    const r = api.ref(storage, path);
    await api.uploadBytes(r, file, {
        contentType: file.type || 'audio/mpeg'
    });
    const url = await api.getDownloadURL(r);
    return { name: file.name || cleanName, url };
}

async function uploadAndSaveMoodSongs() {
    if (!requireAdminForMoodEdit()) return;

    if (!isStorageReady()) {
        showAlert('مكتبة Firebase Storage مش جاهزة. تأكد إنك ضفت firebase-storage في index.html.', 'error');
        setMoodUploadStatus('Storage غير جاهز. راجع إعدادات Firebase Storage.');
        return;
    }

    const input = document.getElementById('moodSongsInput');
    const files = Array.from(input?.files || []);
    if (files.length === 0) {
        showAlert('اختار ملفات أغاني الأول.', 'info');
        return;
    }

    // Limit بسيط لتجنب ملفات ضخمة جداً
    const MAX_MB = 25;
    for (const f of files) {
        const mb = (f.size || 0) / (1024 * 1024);
        if (mb > MAX_MB) {
            showAlert(`الملف "${f.name}" كبير (${mb.toFixed(1)}MB). قلل الحجم أو ارفعه مضغوط.`, 'error');
            return;
        }
    }

    setMoodUploadStatus(`<span class="loading"></span> جاري رفع ${files.length} ملف...`);

    const uploaded = [];
    try {
        // رفع بالترتيب (عشان يفضل نفس ترتيب الاختيار)
        for (let i = 0; i < files.length; i++) {
            const f = files[i];
            setMoodUploadStatus(`<span class="loading"></span> جاري رفع: ${escapeHtml(f.name)} (${i + 1}/${files.length})`);
            const item = await uploadSingleMoodFile(f);
            uploaded.push(item);
        }

        __moodPlaylistDraft = uploaded;

        setMoodUploadStatus(`<span class="loading"></span> جاري حفظ القائمة عند الكل...`);
        const ok = await saveMoodPlaylistToFirestore(uploaded);
        if (!ok) {
            setMoodUploadStatus('فشل حفظ القائمة. راجع Firestore / Rules.');
            showAlert('فشل حفظ القائمة. راجع Console.', 'error');
            return;
        }

        setMoodPlaylist(uploaded);
        setMoodUploadStatus(`تم رفع وحفظ ${uploaded.length} أغنية ✅`);

        try { input.value = ''; } catch (e) {}

        showAlert('تم تحديث قائمة المود عند كل المستخدمين ✅', 'success');
    } catch (e) {
        console.error(e);
        setMoodUploadStatus('حصل خطأ أثناء الرفع. راجع Console.');
        showAlert('حصل خطأ أثناء رفع الأغاني.', 'error');
    }
}

async function saveMoodPlaylistOrder() {
    if (!requireAdminForMoodEdit()) return;

    const list = normalizeMoodPlaylist(__moodPlaylistDraft || []);
    if (list.length === 0) {
        showAlert('القائمة فاضية.', 'info');
        return;
    }

    setMoodUploadStatus(`<span class="loading"></span> جاري حفظ الترتيب عند الكل...`);
    const ok = await saveMoodPlaylistToFirestore(list);
    if (ok) {
        setMoodPlaylist(list);
        setMoodUploadStatus('تم حفظ الترتيب ✅');
        showAlert('تم حفظ ترتيب القائمة عند كل المستخدمين ✅', 'success');
    } else {
        setMoodUploadStatus('فشل حفظ الترتيب.');
        showAlert('فشل حفظ الترتيب. راجع Console.', 'error');
    }
}
...
async function handleAuthStateChanged(user) {
    ...
    if (!user) {
        try { stopMoodMode(); } catch (e) {}
        try { stopMoodPlaylistSync(); } catch (e) {}
    ...
    try { initMoodModeUIOnce(); } catch (e) {}
    try { initMoodPlaylistSync(); } catch (e) {}
    startPresenceLoop();
}
